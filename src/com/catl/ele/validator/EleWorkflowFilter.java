package com.catl.ele.validator;
...
public class EleWorkflowFilter extends DefaultSimpleValidationFilter {

    private static final String CLASSNAME = EleWorkflowFilter.class.getName();
        ...
        try {
            ...
            if(obj instanceof WTPart){
            	...
            	/* if(!isECAD(wtpart.getContainer(), user)&&!isHardwareEngineer(wtpart.getContainer(), user)){
            	 ...
            	 if(!EleCommonUtil.isElePart(wtpart)){
            		 uiValidationStatus = UIValidationStatus.HIDDEN;
            	 }
            }else if(obj instanceof WTDocument){
            	WTDocument doc=(WTDocument) obj;
            	if(!terminateWorkProcess(doc)||!EleCommonUtil.isEleDoc(doc)){
            		 uiValidationStatus = UIValidationStatus.HIDDEN;
            	}
            	/*if(!isECAD(doc.getContainer(), user)&&!isHardwareEngineer(doc.getContainer(), user)){
            		 uiValidationStatus = UIValidationStatus.HIDDEN;
            	}*/
            }
        } catch (Exception e) {
