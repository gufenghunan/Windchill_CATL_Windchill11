...
public class PartPhaseComboBoxDataUtility extends DefaultDataUtility{
	private static Logger logger=Logger.getLogger(PartPhaseComboBoxDataUtility.class.getName());
	...
	public Object getDataValue(String componentId, Object datum, ModelContext modelContext) throws WTException {
		...
		if(obj instanceof WTPart){
			...
			oldPhase = (String) GenericUtil.getObjectAttributeValue(part, "ProductPhase");			
			if(StringUtils.isNotBlank(oldPhase)){
				internalList.add(oldPhase);
				displayList.add(oldPhase);
			}else{
				internalList.add("需求分析");
				displayList.add("需求分析");
			}	       
		}
