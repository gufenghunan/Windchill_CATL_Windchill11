...
public class MaturityConsole implements RemoteAccess {
	...
	public static void step(int step, Scanner sc, RemoteMethodServer rms) throws Exception {
		...
		case 1:
			...
			System.out.println("请输入成熟度");
			sign=sc.next();
			if(toControlpanel(sign)){showControlPanel(sc, rms);return;};
			while(!isTuresign(sign)){
				System.out.println("成熟度只能是1、3、6,请重新输入");
				sign=sc.next();
				if(toControlpanel(sign)){showControlPanel(sc, rms);return;};
			}
			System.out.println("请确认,刷存储库"+param1+"成熟度为"+sign+"(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("washPlatformByLib", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
		...
		case 2:
			...
			System.out.println("请确认,刷产品库"+param1+"成熟度为"+sign+"(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("washPlatformByPro", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		...
		case 3:
			...
			System.out.println("请确认,刷编码前缀为"+param1+"的部件,成熟度为"+sign+"(Y,N)");
			sure=sc.next();
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("washPlatformByPrefix", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		...
		case 4:
			...
			System.out.println("请确认,刷BOM头前缀"+param1+"的部件,成熟度为"+sign+"(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("washPlatformWithChildsByPrefix", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		...
		case 5:
			...
			if(cmd.equals("1")){
				System.out.println("请确认,刷路径"+param1+"文件中的部件,成熟度为"+sign+"(Y,N)");
			}else{
				System.out.println("请确认,刷路径"+param1+"文件中的BOM,成熟度为"+sign+"(Y,N)");
			}
			...
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				if(cmd.equals("1")){
				String msg=(String) rms.invoke("washPlatformByExcel", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
				System.out.println(msg);
				System.out.println();
				}else{
				String msg=(String) rms.invoke("washBomPlatformByExcel", MaturityConsole.class.getName(), null, new Class[]{String.class,String.class},new Object[]{param1,sign});
				System.out.println(msg);
				System.out.println();
				}
			}
		...
		case 6:
			...
			param1=sc.next();
			if(toControlpanel(param1)){showControlPanel(sc, rms);return;};
			System.out.println("请确认,清除存储库"+param1+"成熟度(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("clearPlatformByLib", MaturityConsole.class.getName(), null, new Class[]{String.class},new Object[]{param1});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		...
		case 7:
			...
			param1=sc.next();
			if(toControlpanel(param1)){showControlPanel(sc, rms);return;};
			System.out.println("请确认,清除产品库"+param1+"成熟度(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("clearPlatformByPro", MaturityConsole.class.getName(), null, new Class[]{String.class},new Object[]{param1});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		...
		case 8:
			...
			param1=sc.next();
			if(toControlpanel(param1)){showControlPanel(sc, rms);return;};
			System.out.println("请确认,清除编码前缀为"+param1+"的部件成熟度(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("clearPlatformByPrefix", MaturityConsole.class.getName(), null, new Class[]{String.class},new Object[]{param1});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
    	...
    	case 9:
			...
			param1=sc.next();
			if(toControlpanel(param1)){showControlPanel(sc, rms);return;};
			System.out.println("请确认,清除路径"+param1+"文件中的部件成熟度(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("clearPlatformByExcel", MaturityConsole.class.getName(), null, new Class[]{String.class},new Object[]{param1});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
		    break;
    	case 10:
			System.out.println("请确认,清除所有部件成熟度(Y,N)");
			sure=sc.next();
			if(toControlpanel(sure)){showControlPanel(sc, rms);return;};
			if(sure.toUpperCase().equals("Y")){
				System.out.println("正在执行...");
				String msg=(String) rms.invoke("clearAllPartPlatform", MaturityConsole.class.getName(), null, new Class[]{},new Object[]{});
				System.out.println(msg);
				System.out.println();
			}
			showControlPanel(sc, rms);
    ...
    public static void showControlPanel(Scanner sc, RemoteMethodServer rms) throws Exception{
		...
		System.out.println("------------------------------");
    	System.out.println("1.按存储库名字设置成熟度");
		System.out.println("2.按产品库名字设置成熟度");
		System.out.println("3.按编码前缀设置成熟度");
		System.out.println("4.按BOM结构设置成熟度");
		System.out.println("5.按Excel信息设置成熟度");
		System.out.println("6.按存储库名字清除成熟度");
		System.out.println("7.按产品库名字清除成熟度");
		System.out.println("8.按编码前缀清除成熟度");
		System.out.println("9.按Excel信息清除成熟度");
		System.out.println("10.清除所有部件成熟度");
	...
	public static int setPlatform(WTPart part, String value)
		...
		String platform = IBAUtil.getStringIBAValue(partmaster,"CATL_Maturity");
		if (StringUtils.isEmpty(platform)) {
			IBAUtility iba = new IBAUtility(partmaster);
			if (!StringUtils.isEmpty(value)) {
				iba.setIBAValue("CATL_Maturity", value);
    ...
    public static void clearPartPlatform(WTPart part) throws WTException, WTPropertyVetoException, RemoteException{
    	...
    	WTPartMaster partmaster = part.getMaster();
		 ...
		 if(!StringUtils.isEmpty(platform)){
			 System.out.println("删除成熟度="+part.getNumber());
			 IBAUtility iba = new IBAUtility(partmaster);
			 iba.deleteIBAValueByLogical("CATL_Maturity");
			 iba.updateAttributeContainer(partmaster);
			 iba.updateIBAHolder(partmaster);
		 }
    }
