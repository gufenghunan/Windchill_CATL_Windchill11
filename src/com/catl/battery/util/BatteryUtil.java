...
public class BatteryUtil implements RemoteAccess{
	...
	public static List<WTDocument> getAllTemplate() throws Exception {
		...
		WTDocument doc=getConfigByName(ConstantBattery.config_batterymath_name);
		if(doc!=null){
			pns.add(doc);
		}
		return pns;
	...
	public static void main(String[] args) throws Exception {
		...
		rms.invoke("getAllTemplate", BatteryUtil.class.getName(), null, null, null);
	}
	...
	public static String downloadDocZip(List<WTDocument> docs) throws WTException, IOException, PropertyVetoException {
		for (int i = 0; i < docs.size(); i++) {
			...
			WTDocumentUtil.downloadDoc(doc, wt_home+ConstantBattery.config_battery_temp);
		}
		String filename= "ri_template.zip";
		ZipUtil.fileToZip(wt_home+ConstantBattery.config_battery_temp, wt_home+ConstantBattery.config_battery_temp,filename);
		return ConstantBattery.config_battery_temp.replace("/codebase", "/Windchill")+filename;
	}
	
	public static void emptyTempDir(){
		File file=new File(wt_home+ConstantBattery.config_battery_temp);
		if(file.isDirectory()){
	...
	public static WTDocument getConfigByName(String name)
        ...
        while(qr.hasMoreElements()){
        	...
        	if(TypeIdentifierHelper.getType(cdoc).getTypename().contains(ConstantBattery.TEMPLATE_WTDOCUMENT_TYPE)){
        		wtdoc=cdoc;
        	}
        }
		return wtdoc;
