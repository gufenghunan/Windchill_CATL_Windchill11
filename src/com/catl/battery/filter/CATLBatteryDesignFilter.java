...
public class CATLBatteryDesignFilter extends DefaultSimpleValidationFilter {
	...
	public UIValidationStatus preValidateAction(UIValidationKey key,
		...
		if(persistable instanceof Folder){
			...
			System.out.println("----"+containertype);
				if((containertype.equals("Product")||containertype.equals("产品"))&&folder.getName().equals(ConstantBattery.DOC_FOLDER_NAME.replace("/", ""))){
			...
			if((containertype.equals("Product")||containertype.equals("产品"))&&type.contains(ConstantBattery.WTDOCUMENT_TYPE)){
				WTPrincipal user;
				try {
