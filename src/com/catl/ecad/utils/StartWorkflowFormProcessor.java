...
public class StartWorkflowFormProcessor extends DefaultObjectFormProcessor {

	private static final Logger log = Logger
			.getLogger(StartWorkflowFormProcessor.class.getName());
	...
	public FormResult doOperation(NmCommandBean clientData,
		...
		try {
			...
			if (object instanceof WTPart) {
				...
				boolean a = StartWFFilter.terminateWorkProcess(wtpart);
				if (!a) {
					throw new WTException("流程已存在！");
				}
				startWorkFlow(ECADConst.CREATECOMPONENT_WF, wtpart, null);
				title = "start_workflow_title";
			} else if (object instanceof EPMDocument) {
				EPMDocument epm = (EPMDocument) object;
				boolean a = StartWFFilter.terminateWorkProcess(epm);
				if (!a) {
					throw new WTException("流程已存在！");
				}
				startWorkFlow(ECADConst.PCBDESIGN_WF, epm, null);
				title = "start_pcbworkflow_title";
			}
