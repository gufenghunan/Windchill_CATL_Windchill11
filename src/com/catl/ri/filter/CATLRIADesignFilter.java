...
public class CATLRIADesignFilter extends DefaultSimpleValidationFilter {
	...
	public UIValidationStatus preValidateAction(UIValidationKey key,
		...
		if(persistable instanceof Folder){
			...
			System.out.println("----"+containertype);
				if((containertype.equals("Product")||containertype.equals("产品"))&&(folder.getFolderPath().contains(ConstantRI.DOC_FOLDER_NAMEA.replace("/", "")))){
					if(folder.getDescription().contains(ConstantRI.fodera_description)){
			...
			if((containertype.equals("Product")||containertype.equals("产品"))&&type.contains(ConstantRI.WTDOCUMENT_TYPEA)){
				WTPrincipal user;
				try {
