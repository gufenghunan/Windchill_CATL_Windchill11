package com.catl.ri.riB.service.impl;
...
@Scope("prototype")
@Service("riBSelectMaterialService")
public class SelectMaterialServiceImpl implements SelectMaterialService {
	...
	public JSONArray getRecipeJson(String containerOid,String recipenumber) throws WTException, IOException, PropertyVetoException {
		...
		ApplicationData appdata=WTDocumentUtil.downloadDocPrimary(doc,wt_home+ConstantRI.base_ri_pathB);
		List dbs=new ArrayList();
		List types=new ArrayList();
		if(appdata!=null){
    		String tempfilepath=wt_home+ConstantRI.base_ri_pathB+appdata.getFileName();
    		...
    		if(maps.size()==0){
    			...
    			for (int i = 0; i < maps.size(); i++) {
					...
					if(!StringUtils.isEmpty(pn)){
						WTPart part=CommonUtil.getLatestWTpartByNumber(pn);
						if(part==null){
							nomaterialerror.append(pn).append(",");
							continue;
						}
	...
	public JSONArray getSearchJson(String type) throws WTException {
		String[] searchclfs=ConstantRI.config_searchclfsB.split(",");
		...
		for (int i = 0; i < searchclfs.length; i++) {
			...
			String filePath=wt_home+ConstantRI.config_path_material_xlsxB;
			ExcelUtil.getPartForExcel(filePath, type, searchclfs[i], partForLibary, partlist);
			String[] excipients=ConstantRI.config_other_excipients.split(",");
			List<String> other_excipients=Arrays.asList(excipients);
			if(other_excipients.contains(searchclfs[i])){
				if(searchmap.containsKey(ConstantRI.iba_attr_otherexcipients)){
					List<Map<String,String>> beforelist=searchmap.get(ConstantRI.iba_attr_otherexcipients);
					partlist.addAll(beforelist);
				}
				searchmap.put(ConstantRI.iba_attr_otherexcipients, partlist);
