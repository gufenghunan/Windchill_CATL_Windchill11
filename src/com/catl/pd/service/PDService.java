...
public interface PDService {
	 ...
	 * 生成的excel需要清除计算公式
	 * 创建文档主内容所在临时目录为ConstantPD.base_pd_path/当前用户名/文档名称.xls
	...
	/**
	 * 判断目录ConstantPD.base_pd_path/当前用户是否存在
	 * 如果存在且包含xls文件则返回字符串"已存在"
	 * 如果不存在则创建目录ConstantPD.base_pd_path/当前用户
	 * 拷贝ConstantPD.config_pdmath_xls到目录ConstantPD.base_pd_path/当前用户
	...
	/**
	 * 将页面所填的信息jsonstr处理
	 * 按照配置表ConstantPD.config_pdupdate_xls
	 ...
	 * @throws Exception
	 */
	JSONArray updatePD(String wtDocOid) throws Exception;
