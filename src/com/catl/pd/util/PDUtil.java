...
public class PDUtil implements RemoteAccess{
	...
	public static List<WTDocument> getAllTemplate() throws Exception {
		...
		WTDocument doc=getConfigByName(ConstantPD.config_pdmath_name);
		if(doc!=null){
			pns.add(doc);
		}
		return pns;
	...
	public static void main(String[] args) throws Exception {
		...
		rms.invoke("getAllTemplate", PDUtil.class.getName(), null, null, null);
	}
	...
	public static String downloadDocZip(List<WTDocument> docs) throws WTException, IOException, PropertyVetoException {
		for (int i = 0; i < docs.size(); i++) {
			...
			WTDocumentUtil.downloadDoc(doc, wt_home+ConstantPD.config_pd_temp);
		}
		String filename= "ri_template.zip";
		ZipUtil.fileToZip(wt_home+ConstantPD.config_pd_temp, wt_home+ConstantPD.config_pd_temp,filename);
		return ConstantPD.config_pd_temp.replace("/codebase", "/Windchill")+filename;
	}
	
	public static void emptyTempDir(){
		File file=new File(wt_home+ConstantPD.config_pd_temp);
		if(file.isDirectory()){
	...
	public static WTDocument getConfigByName(String name)
        ...
        while(qr.hasMoreElements()){
        	...
        	if(TypeIdentifierHelper.getType(cdoc).getTypename().contains(ConstantPD.TEMPLATE_WTDOCUMENT_TYPE)){
        		wtdoc=cdoc;
        	}
        }
		return wtdoc;
