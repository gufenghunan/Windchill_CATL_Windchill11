...
public class LoadAttachmentDC1 implements RemoteAccess {
	...
	public static void main(String[] args) throws InvocationTargetException, IOException {
		/*
		 * Scanner scanner = new Scanner(System.in); String methodName =
		 * scanner.nextLine(); System.out.println(methodName);
		 */
		...
		try {
			...
			RemoteMethodServer.getDefault().invoke("loadCADPDF",
					LoadAttachmentDC1.class.getName(), null, argTypes, svrArgs);
		} catch (WTException e) {
			e.printStackTrace();
		}
	...
	public static void updateAttachment(Map<String, File> attachments)
			...
			if (doc != null) {
				...
				if (trv.getName().equals("com.CATLBattery.autocadDrawing")) {
					...
					while (qr.hasMoreElements()) {
						ApplicationData app = (ApplicationData) qr
								.nextElement();

						if (app.getFileName().equals(key + "_RELEASE.pdf")) {
							PersistenceHelper.manager.delete(app);
							logs.add(new String[] { "PDF", "线束", key,
									attachments.get(key).getName(), "删除DC盖章" });
						}

						if (app.getFileName().equals(
								attachments.get(key).getName())) {
							PersistenceHelper.manager.delete(app);
							logs.add(new String[] { "PDF", "线束", key,
									attachments.get(key).getName(), "更新PDF" });
							exist = true;
							// break;
						}
					}
					...
					if (!exist) {
						logs.add(new String[] { "PDF", "线束", key,
								attachments.get(key).getName(), "新增附件" });
					}

					ApplicationData newapp = ApplicationData
							.newApplicationData(ch);
					newapp.setRole(ContentRoleType.SECONDARY);
					String attachmentPath = attachments.get(key)
							.getAbsolutePath();
					// System.out.println("attachmentPath:"+attachmentPath);
					newapp = ContentServerHelper.service.updateContent(ch,
							newapp, attachmentPath);
					newapp = (ApplicationData) PersistenceHelper.manager
							.save(newapp);
				}
			} else {
				...
				if (part != null) {
					...
					while (qr.hasMoreElements()) {
						...
						if (app.getFileName().equals(key + "_RELEASE.pdf")) {
							PersistenceHelper.manager.delete(app);
							logs.add(new String[] { "PDF", "CATIA", key,
									attachments.get(key).getName(), "删除DC盖章" });
						}
					...
					logs.add(new String[] { "PDF", "CATIA", key,
							attachments.get(key).getName(), "系统中PN不存在" });
				}
			}
		}
	}
	...
	public static void loadCADPDF(String filePath) throws Exception {
		...
		try {
			...
			Map<String, File> pdfDC = new HashMap<String, File>();
			for (String keyxx : pdfdoc.keySet()) {
				pdfDC.put(keyxx, pdfdoc.get(keyxx));
			}
			...
			for (String key : caddoc.keySet()) {
				...
				if (part != null) {
					...
					if (doc != null) {
						...
						if (trv.getName().equals(
								"com.CATLBattery.autocadDrawing")) {
							ContentHolder ch = (ContentHolder) doc;
							QueryResult qr = wt.content.ContentHelper.service
									.getContentsByRole(doc,
							...
							ApplicationData newprimaryapp = ApplicationData
									.newApplicationData(ch);
							newprimaryapp.setRole(ContentRoleType.PRIMARY);
							String primaryPath = caddoc.get(key)
									.getAbsolutePath();
							// System.out.println("attachmentPath:"+attachmentPath);
							newprimaryapp = ContentServerHelper.service
									.updateContent(ch, newprimaryapp,
											primaryPath);
							newprimaryapp = (ApplicationData) PersistenceHelper.manager
									.save(newprimaryapp);
							...
							if (pdfdoc.get(key) != null) {
								...
								while (qr1.hasMoreElements()) {
									ApplicationData app = (ApplicationData) qr1
											.nextElement();
									if (app.getFileName().equals(
											pdfdoc.get(key).getName())) {
										PersistenceHelper.manager.delete(app);
										logs.add(new String[] { "DWG", "线束",
												key, caddoc.get(key).getName(),
												"更新DWG、PDF" });
									}
								...
								ApplicationData newsencodaryapp = ApplicationData
										.newApplicationData(ch);
								newsencodaryapp
										.setRole(ContentRoleType.SECONDARY);
								String sencodaryPath = pdfdoc.get(key)
										.getAbsolutePath();
								// System.out.println("attachmentPath:"+attachmentPath);
								newsencodaryapp = ContentServerHelper.service
										.updateContent(ch, newsencodaryapp,
												sencodaryPath);
								newsencodaryapp = (ApplicationData) PersistenceHelper.manager
										.save(newsencodaryapp);
								pdfdoc.remove(key);
							} else {
								logs.add(new String[] { "DWG", "线束", key,
										caddoc.get(key).getName(), "更新DWG" });
							}
						}
						...
						if (pdfdoc.get(key) != null) {
							createDocument(
									part,
									"wt.doc.WTDocument|com.CATLBattery.CATLDocument|com.CATLBattery.autocadDrawing",
									caddoc.get(key).getAbsolutePath(),
									pdfdoc.get(key) == null ? "" : pdfdoc.get(
											key).getAbsolutePath(), key, key);
							logs.add(new String[] {
									"DWG",
									"线束",
									key,
									caddoc.get(key).getName(),
									"新建"
											+ (pdfdoc.get(key) == null ? "无PDF"
													: "有PDF") });
							pdfdoc.remove(key);
						} else {
							logs.add(new String[] { "DWG", "线束", key,
									caddoc.get(key).getName(), "无PDF没有导入" });
						}
					}

				} else {
					logs.add(new String[] { "DWG", "线束", key,
							caddoc.get(key).getName(), "线束没有对应的PN" });
				}
			}

			updateAttachment(pdfdoc);

			for (String keydc : pdfDC.keySet()) {
				WTPart part = getPart(keydc);
				if (part != null) {
					deletePartAtt(part); // DWG与CATIA的PDF同时存在时，Part附件删除
					if (part.getLifeCycleState().getDisplay().equals("已发布")) {
						Image image = PDFSignetUtil.getPrintImange(30, 30);
						String pass = PDFSignetUtil.getEncryptionPW();
						PDFSignetUtil.printSignetAndEncryptionFromPart(part,
								image, pass);
						Set<WTPart> allChildren = new HashSet<WTPart>();
						WTArrayList parents = new WTArrayList();
						parents.add(part);
						WTPartConfigSpec config = WTPartHelper.service
								.findWTPartConfigSpec();
						WTPartStandardConfigSpec standardConfig = config
								.getStandard();
						fetchAllChildren(allChildren, parents, standardConfig);
						for (WTPart child : allChildren) {
							PDFSignetUtil.printSignetAndEncryptionFromPart(
									child, image, pass);
						}

						logs.add(new String[] { "DC盖章", "", keydc, "", "DC盖章成功" });
					} else {
						logs.add(new String[] { "DC盖章", "", keydc, "",
								"PN非已发布的" });
					}
				} else {
					logs.add(new String[] { "DC盖章", "", keydc, "", "系统没有对应的PN" });
				}
			}
		...
		try {
			boolean flag = writer.exportExcelList(rootPath + "/数据导入日志.xlsx",
					"数据导入日志",
					new String[] { "文件格式", "数据类型", "PN", "文件名", "备注" }, logs);
	...
	private static void deletePartAtt(WTPart part) throws WTException, RemoteException {
		String pdf_state = "无";
		String dwg_state = "无";
		...
		while (qr1.hasMoreElements()) {
			...
			TypeDefinitionReadView trv = TypeDefinitionServiceHelper.service
					.getTypeDefView(type);
			if (doc.getNumber().equals(part.getNumber())
					&& trv.getName().equals("com.CATLBattery.autocadDrawing")) {
				dwg_state = "有";
				break;
			}
		}

		QueryResult qr = wt.content.ContentHelper.service.getContentsByRole(
				part, wt.content.ContentRoleType.SECONDARY);
		while (qr.hasMoreElements()) {
			ApplicationData app = (ApplicationData) qr.nextElement();
			if (app.getFileName().toLowerCase()
					.equals(part.getNumber() + ".pdf")) {
				pdf_state = "有";
				if (pdf_state.equals("有") && dwg_state.equals("有")) {
					PersistenceHelper.manager.delete(app); // Part有附件同时关联了AutoCAD
					logs.add(new String[] { "PDF", "CATIA", part.getNumber(),
							app.getFileName(), "删除Part附件" });
				}
			}

			if (app.getFileName().equals(part.getNumber() + "_RELEASE.pdf")) {
				PersistenceHelper.manager.delete(app);
				logs.add(new String[] { "PDF", "CATIA", part.getNumber(),
						app.getFileName(), "删除DC盖章" });
			}
		}
	}

	private static void fetchAllChildren(Set<WTPart> allChildren,
			WTArrayList parents, WTPartStandardConfigSpec standardConfig)
			throws WTException {
		WTArrayList children = new WTArrayList();
		Persistable[][][] linkInfos = WTPartHelper.service.getUsesWTParts(
				parents, standardConfig);
	...
	public static void test() throws Exception {
		...
		createDocument(
				part,
				"wt.doc.WTDocument|com.CATLBattery.CATLDocument|com.CATLBattery.autocadDrawing",
				primary_doc_path, att_doc_path, part.getNumber(),
				part.getNumber());
	}
	...
	private static WTDocument createDocument(WTPart part, String doc_type,
			String primary_doc_path, String att_doc_path, String doc_name,
			String doc_number) throws Exception {
		...
		if (!containerName.equals("电芯") && !containerName.equals("电子电气件")
				&& !containerName.equals("紧固件") && !containerName.equals("原材料")) {
		...
		String containerRef = rf.getReferenceString(ObjectReference
				.newObjectReference(((Persistable) container).getPersistInfo()
						.getObjectIdentifier()));

		WTContainerRef ref = WTContainerRef.newWTContainerRef(container);
		Folder subfolder = wt.folder.FolderHelper.service.getFolder(doc_folder,
				ref);
		String folderRef = new ReferenceFactory()
				.getReferenceString(ObjectReference
						.newObjectReference(((Persistable) subfolder)
								.getPersistInfo().getObjectIdentifier()));

		TypeIdentifier typeidentifier = FdnWTContainerHelper
				.toTypeIdentifier(doc_type);
		...
		document = ApplicationIntegrationFactory.createDocument(typeidentifier,
				input_data, false, SessionHelper.manager.getLocale(), "html");
		...
		ap = ContentServerHelper.service
				.updateContent(document, ap, primary_doc_path);
		ap = (ApplicationData) PersistenceHelper.manager.save(ap);
		document.setFormat(ap.getFormat());
		...
		if (!att_doc_path.equals("")) {
			...
			// System.out.println("attachmentPath:"+attachmentPath);
			newsencodaryapp = ContentServerHelper.service.updateContent(document,
					newsencodaryapp, att_doc_path);
			newsencodaryapp = (ApplicationData) PersistenceHelper.manager
					.save(newsencodaryapp);
		}
		...
		LifeCycleHelper.service.setLifeCycleState(document,
				State.toState("RELEASED"), true);

		return document;
	}

	static WTPart checkOutWTPart(WTPart part) throws WTException,
	...
	static Workable checkInObject(Workable object)
	...
	private static void getAllFile(Map<String, File> pdfdoc,
			Map<String, File> caddoc, File root) {
		if (root.isDirectory()) {
			...
			for (File file : files) {
				if (!file.isDirectory()) {
					if (file.getName().indexOf(".pdf") > 0) {
						...
						String pn = file.getName().substring(0,
	...
	static WTPart getPart(String number) throws WTException {
		...
		SearchCondition sc = new SearchCondition(WTPart.class, WTPart.NUMBER,
				SearchCondition.EQUAL, number.trim());
		qs.appendWhere(sc);
	...
	static WTDocument getLatestDoc(WTDocumentMaster docMaster)
	...
	static WTPart getLatestPart(WTPartMaster partMaster)
			throws PersistenceException, WTException {
		WTPart part = null;
		if (partMaster != null) {
			QueryResult qr = VersionControlHelper.service
