...
function EnableTextBox(){
	...
	//return PTC.wizard.checkRequired(false,false,getMainForm()); // a bug in OOTB Windchill 11.0 F000, use bellow code to fix!
	var result = true;
	var skipRequiredFieldValidationInput = $$("input[id='XWORKS_SKIP_REQUIRED_FIELD_VALIDATION']");
	var needCheckRequired = true;
	if (skipRequiredFieldValidationInput.length > 0) {
		console.log("1. found an id=XWORKS_SKIP_REQUIRED_FIELD_VALIDATION input field in page... ")
		for (var i = 0; i < skipRequiredFieldValidationInput.length; i++) {
			if ("true" == skipRequiredFieldValidationInput[i].value) {
				console.log("2. found an id=XWORKS_SKIP_REQUIRED_FIELD_VALIDATION input field value is true.")
				needCheckRequired = false;
				break;
			}
		}
	} else {
		console.log("Cannot found an id=XWORKS_SKIP_REQUIRED_FIELD_VALIDATION input field in page!")
	}
	if (needCheckRequired) {
		result = PTC.wizard.checkRequired(false,false,getMainForm());
	} else {
		console.log("PTC.wizard.checkRequired() has been skipped!")
	}
	//window.alert(result);
	if (!result) {
		busySign.hide();
