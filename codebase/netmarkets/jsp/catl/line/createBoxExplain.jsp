...
#select_customer .x-panel-body{
 height:45px !important;
}
#step2Column .x-form-field-wrap{
    left: -5px !important;
}
</style>
...
<script type="text/javascript">
	...
	var rt = Ext.data.Record.create([{name: 'name'},{name: 'number'},{name:'quantity'},{name:'packageAsk'},{name:'mtag_content'},{name:'remark'}]);
	var actionTab = 0;
	//第二步的表格
	var step2Column;
	//第二步的表格内容
	...
	Ext.onReady(function() {
		...
		Ext.EventManager.onWindowResize(function(){
			 ...
			 Ext.getCmp("step2Column").setWidth(winWidth*0.99);
			 Ext.getCmp("step2Column").setHeight(winHeight-360);
			 Ext.getCmp("title").setWidth(winWidth);
			 Ext.getCmp("footerpanel").setWidth(winWidth*0.99);
			 Ext.getCmp("fill_customer_detail").setHeight(winHeight-150);
			 Ext.getCmp("fill_customer_detail").setWidth(winWidth*0.99);
		});
		...
		var containerMain = new Ext.Container({
			...
			renderTo:Ext.getBody(),
			width:'99%',
			style:'border:1px solid #c0c0c0;margin:0 auto;min-height:45px'
	    });	
	...
	function submitForm(){
		...
		for (var i = 0; i < total; i++) {
			...
			var packageAskVal = step2ColumnStore.getAt(i).data.packageAsk;
			var mtagcontent = step2ColumnStore.getAt(i).data.mtag_content;
			var remark = step2ColumnStore.getAt(i).data.remark;
			var child = {
				name : nameVal,
				number : numberVal,
				quantity : quantityVal,
				packageAsk : packageAskVal,
				mtagcontent : mtagcontent,
				remark:remark
			};
			//console.log(child);
			childArr.push(child)
		...
		childArr.sort(function(a,b){
				if(a.packageAsk==b.packageAsk){
					var anumber=parseInt(a.number.replace("-M","").replace("-",""));
					var bnumber=parseInt(b.number.replace("-M","").replace("-",""));
					return anumber-bnumber;
				}else{
					return a.packageAsk>b.packageAsk?1:-1;
				}
	...
	function uploadasmform(){
		var oid="<%=oid%>";
		Ext.Ajax.request({
		  	disableCaching:false,
		  	url : "/Windchill/ptc1/boxExplain/getAsmInfo.do?oid="+oid+"&date="+new Date(),
		  	success : function(response, opts) {
		  		var data=JSON.parse(response.responseText);
