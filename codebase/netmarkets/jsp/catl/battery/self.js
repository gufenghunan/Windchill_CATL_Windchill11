var title=["文档属性","设计主界面","选择材料","机械件组合","模切尺寸表","overhang","Summary","BOM","残空间计算","极耳错位","Cell Weight"];
...
var tabbtns=["nbtn,cbtn","bbtn,nbtn,mbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,nbtn,cbtn,sbtn,mbtn,fbtn,ebtn","bbtn,cbtn,mbtn,sbtn,fbtn,ebtn"];
var submitformids=["","inputparams","selectmaterial","mechanicalasm","diecut","overhang","summary","bom","cspace","lugmolposition","cellweight"];
var fillmethodnames=["","fillinputparamsCell","fillMaterialCell","fillMechanicalCell","filldiecutCell","filloverhangCell","fillsummaryCell","fillbomCell","fillcspaceCell","filllugmolpositionCell","fillcellweightCell"];
var matchersheetnames=["","设计主界面","材料数据库","机械件数据库","模切尺寸","Overhang","Summary","BOM","残空间计算","极耳错位","Cell Weight"];
		...
		if(values.battery_name.trim()==""){
			...
			if(current_designname!=designname){
				...
				Ext.Ajax.request({
					...
					async:false,
					url:'/Windchill/ptc1/battery/createDesignExcel.do',
...
function submitform(next_activeindex){
	...
	if(id!=""&&flag){
		...
		Ext.Ajax.request({
			disableCaching : false,
			async:true,
			url : "/Windchill/ptc1/battery/submitForm.do",
			params : {
				wtDocOid:wtoid,
				name:current_name,
				level:current_level,
...
function loadSearchStore(){
	Ext.Ajax.request({
		disableCaching : false,
		url : "/Windchill/ptc1/battery/getSearchJson.do",
		params : {
			type:'name'
		},
		success : function(response,opts) {
	...
	Ext.Ajax.request({
		disableCaching : false,
		url : "/Windchill/ptc1/battery/getMaterialPNJson.do",
	...
	Ext.Ajax.request({
		disableCaching : false,
		url : "/Windchill/ptc1/battery/getAsmPNJson.do",
		success : function(response,opts) {
			  var data=JSON.parse(response.responseText).data;
...
function updateBattery(){
    ...
    mk.show();
	Ext.Ajax.request({
		url:'/Windchill/ptc1/battery/updateBattery.do',
...
summaryjson="",bomjson="",cspacejson="",lugmolpositionjson="",cellweightjson="";
function isFormChange(){
	var flag1=false,flag2=false,flag3=false,flag4=false,flag5=false,flag6=false,flag7=false,flag8=false,flag9=false,flag10=false;
...
function fillinputparamsCell(){
	...
	Ext.Ajax.request({
		url:'/Windchill/ptc1/battery/getPageJson.do',
...
function fillsummaryCell(){
	...
	Ext.Ajax.request({
		...
		success:function(response,opts){
			var data=JSON.parse(response.responseText);
			handlefilldata(data,"input_summary_");
		},
		failure:function(){
			alert('请求失败');
		}
...
function saveDesign(){
	...
	Ext.Ajax.request({
		disableCaching : false,
		async:true,
		url : "/Windchill/ptc1/battery/saveDesign.do",
		params : {
			wtDocOid:wtoid,
			name:current_name,
			level:current_level,
...
function finishForm(){
	   ...
	   Ext.Ajax.request({
		disableCaching : false,
		async:true,
		url : "/Windchill/ptc1/battery/submitvalidateform.do",
		params : {
			wtDocOid:wtoid,
			name:current_name,
			 ...
			 if(validatemsg!=""){
				 alert(validatemsg);
					...
					Ext.Ajax.request({
						disableCaching : false,
						async:true,
						url : "/Windchill/ptc1/battery/outputDoc.do",
						params : {
							wtDocOid:wtoid,
							name:current_name,
							description:checkremark,
...
function loadSummary(id){
	  Ext.Ajax.request({
			url:'/Windchill/ptc1/battery/getSummarySimpleHtml.do',
			params:{templatename:'summary_simple.html',oid:wtDocOid,name:current_name,level:current_level,remark:current_remark},
			success:function(response, opts){
...
function initWindow(){
	 Ext.Ajax.request({
			url:'/Windchill/ptc1/battery/getUserAllFileName.do',
...
function syncTemp(check){
    ...
    mk.show();
	Ext.Ajax.request({
		url:'/Windchill/ptc1/battery/syncTemp.do',
        	...
        	if(obj){
        		...
        		if(style=="显示"){
					...
					if(cache_array!=undefined&&cache_array.length>0){
						...
						m.bigAutocomplete({width:100,data:autoarray,callback:function(data){
        		...
        		if(id=="input_inputparams_"&&cell.region=="B11"){
        			...
        			if(mainB11!='0'){
        				if(value!=mainB11){
        					console.log("设置默认值");
        					checkDefault("设计主界面", cell.region, value, id, false);
        				}
        			}
        			mainB11=value;
        		}
...
function updateSimpleSummary(){
	Ext.Ajax.request({
		url:'/Windchill/ptc1/battery/getPageJson.do',
		params:{wtDocOid:wtoid,sheetname:'Summary',name:current_name,level:current_level,remark:current_remark,date:new Date()},
		success:function(response,opts){
			var data=JSON.parse(response.responseText);
			var data=data.data;
			for(var i=0;i<data.length;i++){
				var cell=data[i];
	        	var inputid="summary_simple_"+cell.region;
	        	var value=cell.value;
	        	var obj=document.getElementById(inputid);
	        	if(obj){
	        		obj.value=handleValue(value);
...
function refreshMathData(id,name){//刷新页面公式数据
	...
	if(id.indexOf("input_")>-1||id.indexOf("sel_")>-1){
		...
		Ext.Ajax.request({
			url:'/Windchill/ptc1/battery/getMathData.do',
...
function exportIfConf(uploadwin){
	var url = website+"/Windchill/ptc1/battery/exportConf.do";
		   ...
		   items:[{
			   ...
			   items:[{
					...
					{    
						...
						handler:function(){
							var form=Ext.getCmp("upload_paramsConf").getForm();
							form.submit({
								url: '/Windchill/ptc1/battery/uploadConf.do',
								method: "POST",
								params:{wtDocOid:wtoid,name:current_name,level:current_level,remark:current_remark,date:new Date()},
...
function checkDefault(sheetname,region,value,idprefix,isconfirm){
	...
	Ext.Ajax.request({
		disableCaching : false,
		async:false,
		url:'/Windchill/ptc1/battery/getDefaultConfig.do',
...
function setCellRemark(cell,value){
	...
	if(value!=""){
		Ext.Ajax.request({
			disableCaching : false,
			async:true,
			url:'/Windchill/ptc1/battery/getRemarkByNumber.do',
			params:{value:value},
			success:function(response, opts){
				var data=JSON.parse(response.responseText);
				if(data.msg!=""){
