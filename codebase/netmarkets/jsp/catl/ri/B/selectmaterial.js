...
function getPartRowJson(oid,rownum){
	 ...
	 console.log(oid);
	  Ext.Ajax.request({
			url:'/Windchill/ptc1/riB/getPartRowJson.do',
...
function createPhantom(jsonstr){
	console.log("创建虚拟件");
	Ext.Ajax.request({
		url:'/Windchill/ptc1/riB/createPhantom.do',
...
function searchmaterialpn(){
	...
	if(material_number!=''){
		Ext.Ajax.request({
			url:'/Windchill/ptc1/riB/getRecipenumbers.do',
...
function searchrecipenumber(){
	...
	if(recipenumber!=""){
		 Ext.Ajax.request({
			url:'/Windchill/ptc1/riB/getRecipeJson.do',
