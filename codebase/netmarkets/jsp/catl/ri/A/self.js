...
var submitformids=["","selectmaterial","mechanicalasm","inputparams","diecut","overhang","summary","bom","cspace","lugmolposition","cellweight"];
var fillmethodnames=["","fillMaterialCell","fillMechanicalCell","fillinputparamsCell","filldiecutCell","filloverhangCell","fillsummaryCell","fillbomCell","fillcspaceCell","filllugmolpositionCell","fillcellweightCell"];
var title=["文档属性","选择材料","机械件组合","设计主界面","模切尺寸表","overhang","Summary","BOM","残空间计算","极耳错位","Cell Weight"];
var matchersheetnames=["","材料数据库","机械件数据库","设计主界面","模切尺寸","Overhang","Summary","BOM","残空间计算","极耳错位","Cell Weight"];
		...
		if(values.battery_name.trim()==""){
			...
			if(current_designname!=designname){
				...
				Ext.Ajax.request({
					...
					async:false,
					url:'/Windchill/ptc1/riA/createDesignExcel.do',
...
function submitform(next_activeindex){
	...
	if(id!=""&&flag){
		...
		Ext.Ajax.request({
			disableCaching : false,
			async:true,
			url : "/Windchill/ptc1/riA/submitForm.do",
			params : {
				wtDocOid:wtoid,
				name:current_name,
				level:current_level,
...
function loadSearchStore(){
	Ext.Ajax.request({
		disableCaching : false,
		url : "/Windchill/ptc1/riA/getSearchJson.do",
		params : {
			type:'name'
		},
		success : function(response,opts) {
	...
	Ext.Ajax.request({
		disableCaching : false,
		url : "/Windchill/ptc1/riA/getAsmPNJson.do",
		success : function(response,opts) {
			  var data=JSON.parse(response.responseText).data;
...
function updateBattery(){
    ...
    mk.show();
	Ext.Ajax.request({
		url:'/Windchill/ptc1/riA/updateBattery.do',
...
function fillinputparamsCell(){
	...
	Ext.Ajax.request({
		url:'/Windchill/ptc1/riA/getPageJson.do',
...
function saveDesign(){
	...
	Ext.Ajax.request({
		disableCaching : false,
		async:true,
		url : "/Windchill/ptc1/riA/saveDesign.do",
		params : {
			wtDocOid:wtoid,
			name:current_name,
			level:current_level,
...
function subfinishForm(){
	   ...
	   Ext.Ajax.request({
		disableCaching : false,
		async:true,
		url : "/Windchill/ptc1/riA/submitvalidateform.do",
		params : {
			wtDocOid:wtoid,
			name:current_name,
			 ...
			 if(validatemsg!=""){
				 alert(validatemsg);
					...
					Ext.Ajax.request({
						disableCaching : false,
						async:true,
						url : "/Windchill/ptc1/riA/outputDoc.do",
						params : {
							wtDocOid:wtoid,
							name:current_name,
							description:checkremark,
...
function loadSummary(id){
	  Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getSummarySimpleHtml.do',
...
function initWindow(){
	 Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getUserAllFileName.do',
...
function syncTemp(check){
    ...
    mk.show();
	Ext.Ajax.request({
		url:'/Windchill/ptc1/riA/syncTemp.do',
...
function refreshMathData(id,name){//刷新页面公式数据
	...
	if(id.indexOf("input_")>-1||id.indexOf("sel_")>-1){
		...
		Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getMathData.do',
...
function exportSheet(uploadwin){
	var url = website+"/Windchill/ptc1/riA/exportSheet.do";
...
function checkDefault(sheetname,region,value,idprefix,isconfirm){
	...
	Ext.Ajax.request({
		disableCaching : false,
		async:false,
		url:'/Windchill/ptc1/riA/getDefaultConfig.do',
...
function setCellRemark(cell,value){
	...
	if(value!=""){
		Ext.Ajax.request({
			disableCaching : false,
			async:true,
			url:'/Windchill/ptc1/riA/getRemarkByNumber.do',
			params:{value:value},
			success:function(response, opts){
				var data=JSON.parse(response.responseText);
				if(data.msg!=""){
