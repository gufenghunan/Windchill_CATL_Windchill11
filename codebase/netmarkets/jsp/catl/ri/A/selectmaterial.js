			...
			},
			{
				...
				listeners:{
					focus:function(){
					   ...
					   console.log(materialpn_searchnumberstore);
				   ...
				   Ext.Ajax.request({
						...
						success:function(response, opts){
							...
							for(var i=0;i<200;i++){
							 ...
							 $("input").bigAutocomplete({width:150,data:[],callback:function(data){}});
						},
						failure:function(){
							alert("请求失败");
						}
...
function handleMaterialInput(id){
	...
	if(dom.getAttribute("search")!=null){
		...
		m.bigAutocomplete({width:150,data:data,callback:function(data){
			 if(type=="name"){
...
function getPartRowJson(oid,rownum){
	 ...
	 console.log(oid);
	  Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getPartRowJson.do',
...
function createPhantom(jsonstr){
	console.log("创建虚拟件");
	Ext.Ajax.request({
		url:'/Windchill/ptc1/riA/createPhantom.do',
...
function searchmaterialpn(){
	...
	if(material_number!=''){
		Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getRecipenumbers.do',
...
function searchrecipenumber(){
	...
	if(recipenumber!=""){
		 Ext.Ajax.request({
			url:'/Windchill/ptc1/riA/getRecipeJson.do',
